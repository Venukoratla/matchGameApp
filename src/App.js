...
class App extends Component {
  ...
  changeImageId = id => {
    const {randomImage, score, isTimeUp} = this.state
    const number = Math.ceil(Math.random() * 30)
    if (id === randomImage.id) {
      this.setState({
        randomImage: imagesList[number],
        score: score + 1,
      })
    } else {
      this.setState({isTimeUp: !isTimeUp})
      clearInterval(this.timerId)
    }
  }
  ...
  getFilteredList = () => {
    ...
    const filteredL = imagesList.filter(
  ...
  render() {
    const {tabIdIs, randomImage, score, time, isTimeUp} = this.state
      ...
      <div>
        ...
        {!isTimeUp && (
          <div className="random-image">
            <div>
              <img className="image" src={randomImage.imageUrl} alt="match" />
            </div>
            <div>
              <ul className="list">
